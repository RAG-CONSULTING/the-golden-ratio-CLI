import { gradeFromScore } from "../engine/ratio-calculator.js";
export function buildFullReport(url, viewport, analyses, sections) {
    const weights = {
        layout: 0.35,
        typography: 0.25,
        spacing: 0.25,
    ...
    const top_strengths = deduped
        ...
        .slice(0, 5);
    const recommendations = top_issues.map((m) => `${m.element} [${m.property}]: ${m.suggestion}`);
    const sectionList = sections ?? [];
    const first_contact = sectionList.length > 0
        ? sectionList[0]
        : {
...
function formatSectionSummary(section) {
    return {
        ...
        score: section.score,
        grade: section.grade,
        category_scores: Object.fromEntries(section.analyses.map((a) => [a.category, { score: a.score, summary: a.summary }])),
...
export function formatFullReport(report, format) {
    if (format === "summary") {
        return JSON.stringify({
            ...
            category_scores: Object.fromEntries(report.analyses.map((a) => [a.category, { score: a.score, summary: a.summary }])),
