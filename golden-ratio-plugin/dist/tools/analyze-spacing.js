...
export function registerAnalyzeSpacing(server) {
    server.tool("analyze_spacing", "Analyze margin and padding relationships for golden ratio harmony. Checks spacing ratios between related elements like sections, cards, and form groups. Returns an annotated screenshot with visual overlays.", {
        ...
        try {
            const { data: measurements, screenshot } = await analyzePage(url, viewport_width, viewport_height, (page) => extractSpacingMeasurements(page, selector, tolerance), (m) => m);
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ error: `Failed to analyze spacing: ${err.message}` }, null, 2),
                    },
                ],
                isError: true,
            };
...
//# sourceMappingURL=analyze-spacing.js.map