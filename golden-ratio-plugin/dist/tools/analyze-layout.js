...
export function registerAnalyzeLayout(server) {
    server.tool("analyze_layout", "Analyze full page layout for golden ratio proportions. Checks content/sidebar width ratios, section height ratios, and major element dimensions. Returns an annotated screenshot with visual overlays.", {
        ...
        try {
            const { data: measurements, screenshot } = await analyzePage(url, viewport_width, viewport_height, (page) => extractLayoutMeasurements(page, tolerance), (m) => m);
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: JSON.stringify({ error: `Failed to analyze layout: ${err.message}` }, null, 2),
                    },
                ],
                isError: true,
            };
...
//# sourceMappingURL=analyze-layout.js.map