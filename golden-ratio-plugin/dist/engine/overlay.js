 ...
 */
export async function captureWithOverlay(page, measurements, scrollY) {
    ...
    `;
        ...
        const placedLabels = [];
        function findNonOverlappingY(x, y, w, h) {
            let candidateY = y;
            for (let attempt = 0; attempt < 4; attempt++) {
                let overlaps = false;
                for (const placed of placedLabels) {
                    if (candidateY < placed.y + placed.h &&
        ...
        // --- Drawing helpers ---
        function drawHorizontalDimLine(container, x, y, width, label, color) {
            // Main line
        ...
        function drawVerticalDimLine(container, x, y, height, label, color) {
            // Main line
            const line = document.createElement("div");
            line.className = "gr-dim-line";
        ...
        function drawRatioTag(container, x, y, m, color) {
        ...
        function resolveRects(elementStr) {
            const selectors = elementStr.split(" / ").map((s) => s.split(" → ")[0].trim());
            const rects = [];
        ...
        function annotateColumnWidth(m, container) {
        ...
        function annotateSectionHeight(m, container) {
            const rects = resolveRects(m.element);
            if (rects.length < 2)
                return;
            const colors = getColors(m);
        ...
        function annotateWidthHeight(m, container) {
            const rects = resolveRects(m.element);
            if (rects.length < 1)
                return;
            const colors = getColors(m);
        ...
        function annotateTypography(m, container) {
            const rects = resolveRects(m.element);
            const colors = getColors(m);
            if (rects.length >= 2) {
        ...
        function annotateSpacing(m, container) {
            const rects = resolveRects(m.element);
            if (rects.length < 1)
                return;
            const colors = getColors(m);
        ...
        function annotateGapPadding(m, container) {
            const parts = m.element.split(" → ");
            if (parts.length < 2) {
                annotateSpacing(m, container);
                return;
            }
            const colors = getColors(m);
            try {
        ...
        for (const m of data.measurements) {
            const selectors = m.element.split(" / ").map((s) => s.split(" → ")[0].trim());
            for (const sel of selectors) {
                try {
                    const el = document.querySelector(sel);
                    if (!el)
        ...
        // --- Draw contextual golden ratio guide lines ---
        // Vertical: at the golden split of the viewport width
        ...
        vLine.style.cssText = `left:${splitX}px;top:${sY}px;width:0;height:${vh}px;border-right:1px dashed rgba(234,179,8,0.3);`;
        overlay.appendChild(vLine);
        // Label the guide line
        const vLabel = document.createElement("div");
        vLabel.className = "gr-dim-label";
        vLabel.style.cssText = `left:${splitX + 4}px;top:${sY + 4}px;background:rgba(234,179,8,0.7);font-size:9px;`;
        vLabel.textContent = `φ split: ${splitX}px | ${vw - splitX}px`;
        overlay.appendChild(vLabel);
        const splitY = Math.round(vh / phi);
        const hLine = document.createElement("div");
        hLine.className = "gr-guide";
        hLine.style.cssText = `left:0;top:${splitY + sY}px;width:100%;height:0;border-bottom:1px dashed rgba(234,179,8,0.3);`;
        overlay.appendChild(hLine);
        const hLabel = document.createElement("div");
        hLabel.className = "gr-dim-label";
        hLabel.style.cssText = `left:4px;top:${splitY + sY + 4}px;background:rgba(234,179,8,0.7);font-size:9px;`;
        hLabel.textContent = `φ split: ${splitY}px | ${vh - splitY}px`;
    ...
    await page.evaluate(() => {
        ...
        document.querySelectorAll("style").forEach((s) => {
            if (s.textContent?.includes("gr-box"))
                s.remove();
        });
    });
