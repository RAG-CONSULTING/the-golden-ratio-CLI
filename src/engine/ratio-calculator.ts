import { PHI, type Measurement } from "./types.js";
...
export function calculateDeviation(ratio: number, target: number = PHI): number {
  if (target === 0) return Infinity;
  return Math.abs((ratio - target) / target) * 100;
}

export function isGoldenRatio(ratio: number, tolerance: number = 0.1): boolean {
  const deviation = calculateDeviation(ratio);
  return deviation <= tolerance * 100;
}

export function scoreMeasurements(measurements: Measurement[]): number {
